...
Now edit the @Gemfile@ to include the following two lines for Haml support:
...
First we setup where we will source of secrets (this will become clear later):

{% highlight ruby %}
# /config/application.rb
module OmniauthGoogleOauth2Example
...
Let's configure the development environment to pull these secrets from a file called @.web_app_secrets@:

{% highlight ruby %}
# /config/environments/development.rb

require 'parseconfig' # *********** DON'T FORGET THIS ************
...
You should now be presented with a page that includes a *Client ID* and *Client secret*. We need to populate these in our @$HOME/.web_app_secrets@ file:

{% highlight ini %}
[omniauth-google-oauth2-example]
login_auth_client_id=<Client ID here>
login_auth_client_secret=<Client secret here>
Application.config.secret_token=make_this_very_random
{% endhighlight %}
